...
Require.prototype.path = function(dest){
    ...
    if (dest.substr(0, 1) === ':'){
        ...
        while (dest.indexOf(this._options.separator.left) > -1){
            var param = dest.substring(dest.indexOf(this._options.separator.left), dest.indexOf(this._options.separator.right)-1);
            var value = param.replace(new RegExp('.', 'g'), '/' + this._options.moduleName + '/');
            dest = dest.replace(new RegExp(this._options.separator.left + param + this._options.separator.right), value);
        }
        dest = $path.join(this._options.root, dest);
    }
